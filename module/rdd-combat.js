...
export class RdDCombat {
  ...
  async proposerAjustementTirLancer(rollData) {
    if (['tir', 'lancer'].includes(rollData.competence.system.categorie)) {
      if (this.defender.isEntite([ENTITE_BLURETTE])) {
        ...
        ChatMessage.create({
          content: await renderTemplate('systems/ctm/templates/chat-info-distance.html', {
            rollData: rollData,
            attacker: _token,
            isVisible: isVisible,
  ...
  async attaque(competence, arme) {
    ...
    if (arme.system.cac == 'empoignade' && this.attacker.isCombatTouche()) {
      ChatMessage.create({
        ...
        content: await renderTemplate('systems/ctm/templates/chat-actor-perte-empoignade.html', {
  ...
  async _onAttaqueParticuliere(rollData) {
    ...
    const choixParticuliere = await ChatMessage.create({
      alias: this.attacker.name,
      whisper: ChatUtility.getWhisperRecipientsAndGMs(this.attacker.name),
      content: await renderTemplate('systems/ctm/templates/chat-demande-attaque-particuliere.html', {
        alias: this.attacker.name,
  ...
  async _chatMessageDefense(paramDemandeDefense, defenderRoll) {
    const choixDefense = await ChatMessage.create({
      ...
      content: await renderTemplate('systems/ctm/templates/chat-demande-defense.html', paramDemandeDefense),
    });
  ...
  async _onAttaqueEchecTotal(attackerRoll) {
    const choixEchecTotal = await ChatMessage.create({
      whisper: ChatUtility.getWhisperRecipientsAndGMs(this.attacker.name),
      content: await renderTemplate('systems/ctm/templates/chat-demande-attaque-etotal.html', {
        attackerId: this.attackerId,
        attacker: this.attacker,
        defenderTokenId: this.defenderTokenId,
        essais: attackerRoll.essais
  ...
  static async displayActorCombatStatus(combat, actor) {
    ...
    ChatUtility.createChatWithRollMode(actor.name, {
      content: await renderTemplate(`systems/ctm/templates/chat-actor-turn-summary.html`, formData)
    });
  }
}
