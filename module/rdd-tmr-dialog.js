...
export class RdDTMRDialog extends Dialog {
  ...
  static async create(actor, tmrData) {
    let html = await renderTemplate('systems/ctm/templates/dialog-tmr.html', tmrData);
  ...
  async _tentativeMaitrise(rencData) {
    ...
    ChatMessage.create({
      ...
      content: await renderTemplate(`systems/ctm/templates/chat-rencontre-tmr.html`, rencData)
    });
  ...
  async _resultatMaitriseCaseHumide(rollData) {
    ...
    ChatMessage.create({
      whisper: ChatUtility.getWhisperRecipientsAndGMs(game.user.name),
      content: await renderTemplate(`systems/ctm/templates/chat-resultat-maitrise-tmr.html`, rollData)
    });
  ...
  async _maitriserTMR(rollData, callbackMaitrise) {
    ...
    const dialog = await RdDRoll.create(this.actor, rollData,
      {
        html: 'systems/ctm/templates/dialog-roll-maitrise-tmr.html',
  ...
  async notifierResonanceSigneDraconique(coord) {
    if (this.actor.isResonanceSigneDraconique(coord)) {
      ChatMessage.create({
        whisper: ChatUtility.getWhisperRecipientsAndGMs(game.user.name),
        content: await renderTemplate(`systems/ctm/templates/chat-signe-draconique-resonance.html`, { alias: this.actor.name, typeTMR: TMRUtility.getTMRType(coord) })
      });
    }
  }
